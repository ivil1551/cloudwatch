...
class CloudwatchHandler(logging.Handler):
    ...
    def __init__(self,
        ...
        try:
            ...
            response = self.logs.describe_log_streams(logGroupName=self.log_group, logStreamName=self.log_stream)
            log_streams = response['logStreams']
            if log_streams:
                self.next_sequence_token = log_streams[0]['uploadSequenceToken'] if 'uploadSequenceToken' in \
                                                                                    log_streams[0] else None
